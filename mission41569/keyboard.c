	...
	)
{
	...
	switch (KeyEventRecord.VirtualKeyValue) {
		...
		//增加空格事件，提高当前线程（除了2号）的优先级，最高为8（为了测试）
		case VK_SPACE:
			if (PspCurrentThread->Priority > 0 && PspCurrentThread->Priority < 8 && ObGetObjectId(PspCurrentThread) != 2) {
				PsSetThreadPriority(CURRENT_THREAD_HANDLE, 8);
			}
			break;
