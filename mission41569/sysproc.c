	...
	)
{
	...
	for (;;){
		...
		if (0 == stricmp(Line, "ver")) {
			...
			continue;
		} else if (0 == stricmp(Line, "up")) {//增加命令up
...
//下面增加一个up命令，用于提升线程优先级：up ID
PRIVATE
VOID
ConsoleCmdUp(//实现up命令函数
	IN HANDLE StdHandle,
	IN PCSTR Arg
	)
{
...
--*/
{
	...
	STATUS Status;
	//获取线程  
	Status = ObRefObjectByHandle(CURRENT_THREAD_HANDLE, PspThreadType, (PVOID*)&Thread);
	if (!EOS_SUCCESS(Status)) {
		return Status;
	}
	...
	for (i = 0; ; i++) {
		...
		//打印内容的修改
		fprintf(pThreadParameter->StdHandle, "Thread %d , ID:%d, Priority:%d, InitTicks: %u , UsedTicks:%u, RemainderTicks:%d ",
			pThreadParameter->Y, ObGetObjectId(PspCurrentThread), Priority, 
			Thread->InitTicks, 
			Thread->UsedTicks, 
			Thread->RemainderTicks);
...
--*/
{
	...
	for (i = 0; i < 10; i++) {
		...
		//初始化增加的两个变量
		Status = ObRefObjectByHandle(ThreadHandleArray[i], PspThreadType, (PVOID*)&Thread);
		Thread->InitTicks = 100;
		Thread->UsedTicks = 0;
