...
{  /*
	...
	*/
	...
	/*
	//-----------------------------3.4.2-------------------------------------
	//-------------使 FatWriteFile 函数写入文件的数据可以跨越一个扇区的边界--------------
	//一次最多写入256，扇区大小512，做最多跨越1个扇区
	//Vcb->Bpb.BytesPerSector;是扇区大小
	STATUS Status;
	...
	ULONG OffsetInSector;
	//flag
	ULONG ByteWrittenTemp = 0;//已写入字节数量
	
	ULONG i;

	// 写入的起始位置不能超出文件大小（并不影响增加文件大小或增加簇）
	if (Offset > File->FileSize)
		return STATUS_SUCCESS;
	...
	{	
		Status = IopReadWriteSector(Vcb->DiskDevice,
			...
			FALSE);
		//修改已经写入的字节数
		ByteWrittenTemp = BytesToWrite;
		if (!EOS_SUCCESS(Status))
			return Status;
	}
		//如果终点超出扇区，就只先写入本扇区最大写入量Vcb->Bpb.BytesPerSector - OffsetInSector
		else
		{	
			Status = IopReadWriteSector(Vcb->DiskDevice,
				FirstSectorOfCluster,
		...
		//修改已经写入的字节数和扇区偏移地址
			ByteWrittenTemp += Vcb->Bpb.BytesPerSector - OffsetInSector;
			OffsetInSector = 0;
		//因为一个簇只有一个扇区，所以新增加一个簇
		//新增一个簇，用于在下次循环中存入数据。与前文类似，簇链指针后移，插入新簇。
			PrevClusterNum = CurrentClusterNum;
			CurrentClusterNum = FatGetFatEntryValue(Vcb, PrevClusterNum);//根据当前簇号得到下一个簇号，因为没有下一个簇，所以值大于等于0xFF8
			//确保下一个簇是空
			if (CurrentClusterNum >= 0xFF8) {
				// 为文件分配一个空闲簇
				FatAllocateOneCluster(Vcb, &NewClusterNum);
				// 将新分配的簇安装到当前簇后面（也就是簇链结尾）
				FatSetFatEntryValue(Vcb, PrevClusterNum, NewClusterNum);//如果新簇不是第一个簇。把新簇链接到当前簇后面
				// 让CurrentClusterNum指针指向新簇，用于下次循环
				CurrentClusterNum = NewClusterNum;
			}
		
		//接下来对剩余的需要跨扇区的部分进行处理
		//其中，“偏移+需要写入字节数-已经写入字节数”就是本次全部写入的终点地址
		//Vcb->Bpb.BytesPerSector是扇区字节数	
		...
		FirstSectorOfCluster = Vcb->FirstDataSector + (CurrentClusterNum - 2) * Vcb->Bpb.SectorsPerCluster;
			...
			Status = IopReadWriteSector(Vcb->DiskDevice,
				FirstSectorOfCluster,
				OffsetInSector,//已经修改
				(PCHAR)(Buffer + ByteWrittenTemp),
				BytesToWrite - ByteWrittenTemp,
				FALSE);
	...
	*/
	...
	//-----------------------------3.4.3-------------------------------------
	//-------------使 FatWriteFile 函数写入文件的数据可以跨越多个扇区的边界 --------------
	STATUS Status;
	...
	//分配了新簇，计算扇区内的字节偏移，
		while (1) {
		if (ByteWrittenTemp >= BytesToWrite)break;//写完了
		//计算当前簇的第一个扇区的扇区号。簇从2开始计数。
		FirstSectorOfCluster = Vcb->FirstDataSector + (CurrentClusterNum - 2) * Vcb->Bpb.SectorsPerCluster;
		//如果终点小于等于扇区字节数，就能一次全部写入扇区
		//其中，“偏移+需要写入字节数-已经写入字节数”就是本次全部写入的终点地址
		//Vcb->Bpb.BytesPerSector是扇区字节数
		if (OffsetInSector + BytesToWrite - ByteWrittenTemp <= Vcb->Bpb.BytesPerSector)
		{	
			Status = IopReadWriteSector(Vcb->DiskDevice,
				FirstSectorOfCluster,
				OffsetInSector,
