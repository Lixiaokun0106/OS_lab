...
LIST_ENTRY RequestListHead;	// 对磁盘设备读写请求的队列。//请求链表的链表头
...
--*/
{
	...
	PsWaitForMutex(&DiskScheduleMutex, INFINITE);	// 进入临界区//P
	...
	PsReleaseMutex(&DiskScheduleMutex);		// 退出临界区//V
...
--*/
{
	...
	if (ListIsEmpty(&RequestListHead)) {
		...
		//
		pNextRequest = IopDiskSchedule();//这里是磁盘调度算法
...
// 块设备使用的唯一的一个缓冲区（静态分配）。
//
PRIVATE BYTE BlockDeviceBuffer[512];//只对一个块设备进行管理


STATUS
IopReadWriteSector(
	IN PDEVICE_OBJECT Device,//块设备
	IN ULONG SectorNumber,//扇区号
	IN ULONG ByteOffset,//读写的字节在扇区中的偏移
	IN OUT PVOID Buffer,//指向缓冲区
	IN ULONG BytesToRw,//读写的字节数，与偏移量相加不能超过扇区大小
	IN BOOL Read//是否进行读操作
	)
/*++
...
--*/
{
	...
	ASSERT(NULL != Device && Device->IsBlockDevice);//对相关限定条件进行判断
	...
	PsWaitForMutex(&Device->Mutex, INFINITE);	// 进入临界区//相当于P操作
	...
	if (Read) {
		...
		// 读缓存
		//
		memcpy(Buffer, BlockDeviceBuffer + ByteOffset, BytesToRw);//读取指定字节
...
RETURN:
	...
	PsReleaseMutex(&Device->Mutex);		// 退出临界区//相当于V操作
	
	//
	// 由磁盘调度算法从请求队列中选择要处理的下一个请求，并开始处理。
	//
	IopProcessNextRequest(pCurrentRequest);//进行磁盘请求调度
