	...
	)
{
	...
	//创建文件对象
	Status = IoCreateFile( (PSTR)FileName,//全路径
						   DesiredAccess,//访问方式
						   ShareMode,
						   CreationDisposition,
						   FlagsAndAttributes,
...
ReadFile(
	...
	IN ULONG NumberOfBytesToRead,//读取的字节大小
...
EOSAPI
ULONG
SetFilePointer(//指向文件末尾
	IN HANDLE FileHandle,
	IN LONG DistanceToMove,//移动的字节数
	IN ULONG MoveMethod//指针移动的起始位置
	)
{
	STATUS Status;
