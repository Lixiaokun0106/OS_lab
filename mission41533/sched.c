...
// 下标为n的链表对应优先级为n的就绪队列。
//
LIST_ENTRY PspReadyListHeads[32];//32个优先级的队列的入口

//
// 32位就绪位图。
// 如果位图的第n位为1，则表明优先级为n的就绪队列非空。
//
volatile ULONG PspReadyBitmap = 0;//32位的就绪位图，按照优先级由高到低进行扫描
...
--*/
{
	...
	// 在此添加代码，实现时间片轮转调度算法。
	//
	if (NULL != PspCurrentThread && Running == PspCurrentThread->State) {//正在运行的线程才调用时间片轮转
		// 在此实现时间片轮转调度算法
		//时间片减少1
		PspCurrentThread->RemainderTicks--;
		if(PspCurrentThread->RemainderTicks==0)//时间片耗尽
		{
		//重新分配时间片
		PspCurrentThread->RemainderTicks=TICKS_OF_TIME_SLICE;
		//若存在相同优先级的就绪线程，则将该线程放至就绪队列末尾
