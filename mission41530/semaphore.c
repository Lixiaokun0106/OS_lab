...
--*/
{
	...
	*/
	...
	if(Semaphore->Count>0)
		...
		{
		Status=PspWait(&Semaphore->WaitListHead, Milliseconds);//阻塞线程执行
		}
	...
	if (Semaphore->Count + ReleaseCount > Semaphore->MaximumCount) {
		...
		*/
		...
		for(i=0;i<ReleaseCount;i++)
			{
			if(ListIsEmpty(&Semaphore->WaitListHead)){break;}
			PspWakeThread(&Semaphore->WaitListHead, STATUS_SUCCESS);
			}
		if(i==ReleaseCount){Semaphore->Count=0;}
		else if(i<ReleaseCount){Semaphore->Count=ReleaseCount-i+(Semaphore->Count);}
		...
		*/
		//定义一个变量来记录增加之前的值
		int PreviousSemaphoreCount = Semaphore->Count;
		while(!ListIsEmpty(&Semaphore->WaitListHead)&&ReleaseCount){
        //循环体实现批量释放的功能
			PspWakeThread(&Semaphore->WaitListHead,STATUS_SUCCESS); //唤醒该线程
				PspThreadSchedule();  //线程调度函数
				ReleaseCount--;  //将释放量减少1
		}
		Semaphore->Count=PreviousSemaphoreCount+ReleaseCount; 
        //如果有剩余加到信号量上
