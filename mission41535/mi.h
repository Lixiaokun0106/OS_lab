...
// 虚拟地址区域描述结构体。
//
typedef struct _MMVAD{//描述虚拟地址空间内那些地址被使用
	ULONG_PTR StartingVpn;//开始虚页框号
	ULONG_PTR EndVpn;//结束虚页框号
	LIST_ENTRY VadListEntry;//链表入口
}MMVAD, *PMMVAD;


typedef struct _MMVAD_LIST{//描述进程使用的地址空间
...
//
typedef struct _MMPAS {//与每个进程相对应
	...
	ULONG_PTR PfnOfPteCounter;//记录页目录表中对应页表的有效项的个数
}MMPAS;
